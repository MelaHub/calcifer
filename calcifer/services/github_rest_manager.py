...
class GithubRestManager(RestPager):
    ...
    def add_protections(self, github_org: str, github_repo_name: str, main_branch: str, protections: dict) -> None:
        response = requests.put(f'{self.url}https://api.github.com/repos/{github_org}/{github_repo_name}/branches/{main_branch}/protection', json=protections, auth = HTTPBasicAuth(self.user, self.token.get_secret_value()))
